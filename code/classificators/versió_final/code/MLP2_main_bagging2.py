...
def run_experiment(embedder, data_mode, name_prefix):
    ...
    if data_type == 'ALL':
        original_data, balanced_data, target_col = load_and_balance_data_all(extra)
    else:
    ...
    X, y, categorical_cols, text_cols, og_indices, txt_shape = prepare_features(
        balanced_data, original_data, embedder=embedder
    )
    ...
    if data_mode == 'cat':
        ...
        X = X[:, txt_shape:]  # Mantener solo las características categóricas
        txt_shape = 0  # No hay características de texto
        ...
        X = X[:, :txt_shape]  # Mantener solo las características de texto
    ...
    wandb.save('/fhome/amir/TFG/code/classificators/DL/MLP2_main_bagging.py')
    wandb.finish()
...
if __name__ == "__main__":
    ...
    author = 'RPA'
    name_prefix_base = f'MLP2_{author}_BAGGING'
    ...
    root_dir = '/fhome/amir/TFG/results/embedder_experiment'
    for mode, rows in results_dict.items():
        df = pd.DataFrame(rows, columns=['embedder', 'recT', 'recF', 'prec'])

        if mode == 'cat':
            filename = f'categorical_{author}.csv'
        elif mode == 'txt':
            filename = f'text_only_{author}.csv'
        elif mode == 'all':
            filename = f'all_{author}.csv'
