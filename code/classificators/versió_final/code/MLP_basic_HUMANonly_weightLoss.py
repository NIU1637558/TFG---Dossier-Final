...
def evaluate_model(model, loader, criterion, device, return_probs= False, weighted_loss=False):
    model.eval()
    running_loss = 0.0
    correct = 0
    total = 0
    ...
    with torch.no_grad():
        for inputs, labels in loader:
            ...
            if weighted_loss:
                ...
                # Si no se usa weighted loss, aplicar sigmoid
...
def evaluate_and_save_best_model_outputs(best_model, test_dataset, y_test, balanced_data, original_data, class_names, target_col, name_model, pos_weight):
...
def train_epoch(model, train_loader, criterion, optimizer, device, weighted_loss):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
...
def single_training_loop(X, y, args, balanced_data, original_data, nepoch, weighted_loss=False):
    ...
    pos = class_counts[1]
    factor_w = 0.8
    pos_weight = torch.tensor([(neg / pos)*factor_w], dtype=torch.float)
    ...
    for epoch in range(nepoch):
        # 1. train epoch
        train_loss, train_acc = train_epoch(model, train_loader, criterion, optimizer, device, weighted_loss )
    ...
    return metrics, model, test_dataset, y_test, pos_weight
...
def main():
    ...
    metrics, model, test_dataset, y_test, pos_weight = single_training_loop(X, y, args, balanced_data, original_data, nepoch, weighted_loss=False)
    sys.stderr.write('---------------------------------------------------------------------\n')
    
    ## ------------- 4. Save metrics and predictions --------------------##
    sys.stderr.write(" -------------------- 4. Save metrics and predictions --------------------\n")
    model_path = evaluate_and_save_best_model_outputs(model, test_dataset, pd.Series(y_test), balanced_data, original_data, class_names, target_col, name, pos_weight)
